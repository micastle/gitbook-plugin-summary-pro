...
const getPaths = (config, pattern) =>
  glob.sync(
    ...
    {
      cwd: config.root,
      ignore: [ 'node_modules/**', '_book/**', 'styles/**', 'layouts/**', 'images/**', 'assets/**', '**/images', '**/assets' ],
      nosort: true
    }
  )
...
const writeSummaryFile = config => summaryContent => {
  var title = config.options.value.firstpage ? config.options.value.firstpage.title : config.bookTitle.value
  fs.writeFileSync(`${config.root}/${config.summaryFilename}`, `- [${title}](README.md)` + '\n' + summaryContent, { encoding: 'utf8' })
}
